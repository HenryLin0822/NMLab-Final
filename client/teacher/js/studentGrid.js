...
class StudentGrid {
    constructor() {
        ...
        // ADD FACE RECOGNITION SUPPORT - ADD THESE LINES
        this.faceRecognitionEnabled = false;
        this.faceRecognitionAlerts = new Map();
        this.faceRegisteredStudents = new Set();
        this.faceVerificationHistory = new Map();
...
// ===== NEW METHOD: Check if gaze alert should be throttled =====
    ...
    setupEventListeners() {
        ...
        document.addEventListener('faceRecognitionStatus', (event) => {
            this.handleFaceRecognitionStatus(event.detail);
        });
        
        document.addEventListener('studentFaceRecognitionUpdate', (event) => {
            this.updateStudentFaceRecognition(event.detail);
        });
        
        document.addEventListener('faceRecognitionAlert', (event) => {
            this.handleFaceRecognitionAlert(event.detail);
        });
        
        document.addEventListener('studentFaceRegistered', (event) => {
            this.handleStudentFaceRegistered(event.detail);
        });
    ...
    addStudentToGrid(studentInfo) {
        ...
        this.students.set(studentInfo.socketId, {
            ...
            // ADD FACE RECOGNITION PROPERTIES
            faceRecognitionData: null,
            faceRecognitionHistory: studentInfo.faceRecognitionHistory || [],
            faceRegistered: studentInfo.faceRegistered || false
    ...
    removeStudent(studentInfo) {
        ...
        this.faceRecognitionAlerts.delete(studentInfo.studentId); // ADD THIS
        this.faceRegisteredStudents.delete(studentInfo.socketId); // ADD THIS
    ...
    assignStudentToSlot(studentSocketId, slotNumber) {
        ...
        // CREATE FACE RECOGNITION INDICATOR - ADD THIS AFTER AI INDICATOR
        if (this.faceRecognitionEnabled) {
            ...
            faceIndicator.innerHTML = `
                <div class="face-verification">👤 Waiting...</div>
                <div class="face-confidence">-</div>
            `;
            faceIndicator.style.display = 'block';
            infoArea.appendChild(faceIndicator);
        }
    ...
    processAlert(alertData, type) {
        ...
        if (student && student.slotNumber) {
            ...
            } else if (type === 'face') { // ADD THIS
                this.showFaceRecognitionAlert(student.slotNumber, alertData.alert);
        }
        }
        ...
        // UPDATE THE LOG ALERT SECTION
        const emojiMap = { gaze: '👀', ai: '🤖', face: '👤' };
    ...
    updateDetailViewInfo(student) {
        ...
        // Update AI detection info
        this.updateDetailAIInfo(student);
        
        // ADD THIS LINE - Update face recognition info
        this.updateDetailFaceInfo(student);
    ...
    updateDetailAlerts(student) {
        ...
        const faceAlert = this.faceRecognitionAlerts.get(studentId); // ADD THIS
        ...
        // ADD FACE RECOGNITION ALERT
        if (faceAlert) {
            const alertTime = new Date(faceAlert.timestamp);
            const timeStr = Utils.formatTime(alertTime);
            
            alertsHTML += `
                <div class="alert-item ${faceAlert.alert.severity}">
                    <div class="alert-message">👤 ${faceAlert.alert.message}</div>
                    <div class="alert-time">${timeStr}</div>
                </div>
            `;
        }
    ...
    getGridState() {
        return {
            ...
            students: Array.from(this.students.values()).map(student => ({
                ...
                faceVerification: student.faceRecognitionData?.face_verification,
                faceConfidence: student.faceRecognitionData?.confidence,
    ...
    handleFaceRecognitionStatus(status) {
        this.faceRecognitionEnabled = status.enabled;
        Utils.log(`Face recognition ${status.enabled ? 'enabled' : 'disabled'}`);
        
        // Update UI for all existing students
        this.students.forEach(student => {
            if (student.slotNumber) {
                this.updateFaceIndicatorVisibility(student.slotNumber);
            }
        });
    }

    // Update student face recognition data
    updateStudentFaceRecognition(faceData) {
        const student = this.students.get(faceData.studentSocketId);
        if (!student || !student.slotNumber) return;
        
        console.log('🔍 Updating face recognition data for student:', student.name, faceData.faceRecognitionData);
        
        // Store face recognition data
        student.faceRecognitionData = faceData.faceRecognitionData;
        
        // Update face recognition history
        if (!student.faceRecognitionHistory) student.faceRecognitionHistory = [];
        student.faceRecognitionHistory.push({
            timestamp: faceData.timestamp,
            verification: faceData.faceRecognitionData.face_verification,
            confidence: faceData.faceRecognitionData.confidence
        });
        
        // Keep only last 10 results
        if (student.faceRecognitionHistory.length > 10) {
            student.faceRecognitionHistory.shift();
        }
        
        // Update face indicator in grid
        this.updateFaceIndicator(student.slotNumber, faceData.faceRecognitionData);
        
        // Update slot visual state based on verification
        this.updateSlotFaceStatus(student.slotNumber, faceData.faceRecognitionData);
        
        // If this is the student in detail view, update detail face info
        if (this.currentDetailStudent === faceData.studentSocketId) {
            this.updateDetailFaceInfo(student);
        }
    }

    // Handle face recognition alerts
    handleFaceRecognitionAlert(alertData) {
        this.faceRecognitionAlerts.set(alertData.studentId, alertData);
        
        // Store in other students alerts for detail view
        this.otherStudentAlerts.set(
            `${alertData.studentId}-face-${Date.now()}`, 
            alertData
        );
        
        this.processAlert(alertData, 'face');
    }
    ...
    handleStudentFaceRegistered(data) {
        ...
        if (student) {
            ...
            this.faceRegisteredStudents.add(data.studentSocketId);
            
            // Update slot visual state
            if (student.slotNumber) {
                this.updateSlotFaceRegistrationStatus(student.slotNumber, true);
            }
            
            Utils.log(`Face registered for ${data.studentName}`);
        }
    }

    // Update face recognition indicator
    updateFaceIndicator(slotNumber, faceData) {
        if (!this.faceRecognitionEnabled) return;
        
        const slotElement = this.gridContainer.querySelector(`[data-slot="${slotNumber}"]`);
        if (!slotElement) return;
        
        let faceIndicator = slotElement.querySelector('.face-indicator');
        if (!faceIndicator) {
            // Create face indicator if it doesn't exist
            const infoArea = slotElement.querySelector('.info-area');
            if (infoArea) {
        ...
        const verificationElement = faceIndicator.querySelector('.face-verification');
        const confidenceElement = faceIndicator.querySelector('.face-confidence');
        
        if (verificationElement && confidenceElement) {
            // Update verification with emoji and text
            const verificationMap = {
                'match': '✅ Identity Verified',
                'no_match': '❌ Identity Mismatch',
                'no_face': '👤 No Face',
                'not_registered': '📷 Not Registered',
                'error': '❌ Error'
            };
            
            verificationElement.textContent = verificationMap[faceData.face_verification] || '❓ Unknown';
            
            // Update confidence
            const confidence = Math.round((faceData.confidence || 0) * 100);
            confidenceElement.textContent = `Confidence: ${confidence}%`;
            
            // Add visual styling based on verification result
            faceIndicator.className = 'face-indicator';
            faceIndicator.classList.add(`face-${faceData.face_verification}`);
            faceIndicator.style.display = 'block';
        }
    }

    // Update slot face status styling
    updateSlotFaceStatus(slotNumber, faceData) {
        const slotElement = this.gridContainer.querySelector(`[data-slot="${slotNumber}"]`);
        if (!slotElement) return;
        
        // Remove existing face status classes
        slotElement.classList.remove('face-verified', 'face-mismatch', 'face-not-registered');
        
        // Add appropriate class based on verification result
        if (faceData.face_verification === 'match') {
            slotElement.classList.add('face-verified');
        } else if (faceData.face_verification === 'no_match') {
            slotElement.classList.add('face-mismatch');
        } else if (faceData.face_verification === 'not_registered') {
            slotElement.classList.add('face-not-registered');
        }
    }

    // Show/hide face indicator based on recognition status
    updateFaceIndicatorVisibility(slotNumber) {
        const slotElement = this.gridContainer.querySelector(`[data-slot="${slotNumber}"]`);
        if (!slotElement) return;
        
        const faceIndicator = slotElement.querySelector('.face-indicator');
        if (faceIndicator) {
            faceIndicator.style.display = this.faceRecognitionEnabled ? 'block' : 'none';
        }
    }

    // Show face recognition alert overlay
    showFaceRecognitionAlert(slotNumber, alert) {
        this.showAlert(slotNumber, alert, 'face');
    }

    // Update detail view face recognition information
    updateDetailFaceInfo(student) {
        // Check if detail view elements exist
        const detailFaceVerification = document.getElementById('detailFaceVerification');
        const detailFaceConfidence = document.getElementById('detailFaceConfidence');
        const detailFaceHistory = document.getElementById('detailFaceHistory');
        const detailFaceRegistered = document.getElementById('detailFaceRegistered');
        
        if (!detailFaceVerification) {
            console.log('⚠️ Face recognition detail view elements not found in HTML');
            return;
        }
        
        const currentFace = detailFaceVerification.parentElement;
        
        if (student.faceRecognitionData) {
            console.log('📊 Updating detail face info:', student.faceRecognitionData);
            
            // Update current face verification
            const verificationMap = {
                'match': '✅ Identity Verified',
                'no_match': '❌ Identity Mismatch',
                'no_face': '👤 No Face Detected',
            ...
            detailFaceVerification.textContent = 
                verificationMap[student.faceRecognitionData.face_verification] || '❓ Unknown';
            
            const confidence = Math.round((student.faceRecognitionData.confidence || 0) * 100);
            if (detailFaceConfidence) {
                detailFaceConfidence.textContent = `Confidence: ${confidence}%`;
            }
            
            // Update face verification styling
            if (currentFace) {
                currentFace.className = `current-face face-${student.faceRecognitionData.face_verification}`;
            }
        } else {
            detailFaceVerification.textContent = '👤 Waiting for data...';
            if (detailFaceConfidence) {
                detailFaceConfidence.textContent = 'Confidence: -';
            }
            if (currentFace) {
                currentFace.className = 'current-face';
            }
        }
        
        // Update face registration status
        if (detailFaceRegistered) {
            detailFaceRegistered.textContent = student.faceRegistered ? 'Yes' : 'No';
        }
        
        // Update face recognition history
        this.updateFaceHistory(student);
    }

    // Update face recognition history in detail view
    updateFaceHistory(student) {
        const detailFaceHistory = document.getElementById('detailFaceHistory');
        if (!detailFaceHistory) {
            console.log('⚠️ Face history element not found in HTML');
            return;
        }
        
        const history = student.faceRecognitionHistory || [];
        
        if (history.length === 0) {
            detailFaceHistory.innerHTML = '<div class="no-alerts">No face recognition data yet</div>';
            return;
        }
        ...
        const historyHTML = history.slice(-10).reverse().map(item => {
            const time = new Date(item.timestamp);
            const timeStr = Utils.formatTime(time);
            const verification = item.verification.charAt(0).toUpperCase() + item.verification.slice(1).replace('_', ' ');
            
            return `
                <div class="face-history-item">
                    <span class="face-history-verification">${verification}</span>
                    <span class="face-history-time">${timeStr}</span>
                </div>
            `;
        }).join('');
        
        detailFaceHistory.innerHTML = historyHTML;
    }

    // Update slot face registration status
    updateSlotFaceRegistrationStatus(slotNumber, isRegistered) {
        const slotElement = this.gridContainer.querySelector(`[data-slot="${slotNumber}"]`);
        if (!slotElement) return;
        
        if (isRegistered) {
            slotElement.classList.remove('face-not-registered');
            slotElement.classList.add('face-registered');
        } else {
            slotElement.classList.add('face-not-registered');
            slotElement.classList.remove('face-registered');
        }
}
}
